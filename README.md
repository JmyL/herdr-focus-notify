...
The notification body is `{workspace} · {preview}` (truncated to 120 characters). `workspace` is the Herdr workspace label with a navigator-style `token:\s` prefix removed (for example `project: ree-drive` → `ree-drive`), falling back to the cwd basename. For Cursor, `preview` is the first and last prose lines of the latest assistant turn (about 40 characters each, joined with ` … `), skipping fenced code and code-like lines; a single prose line is used alone. If no prose remains, it falls back to the terminal/session title. The Cursor session id is taken from the live chat DB opened by that pane's agent process when possible (so `/clear` does not leave a stale Herdr `agent_session`), then falls back to Herdr's reported session. Other agents always use the session title.
