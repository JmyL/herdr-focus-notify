...
The notification title is `[{workspace}] {agent} finished` / `[{workspace}] {agent} needs attention`. `workspace` is the Herdr workspace label with a navigator-style `token:\s` prefix removed (for example `project: ree-drive` → `ree-drive`), falling back to the cwd basename. The body is `{preview}` only (truncated to 280 characters). For Cursor, `preview` is the first and last prose lines of the latest assistant turn (up to 100 characters each, as `{first}...\n...\n{last}...` when truncated), after stripping inline markdown and skipping fenced/code-like lines; a single prose line is used alone. When that turn asks via `AskQuestion` or `CreatePlan`, the body prefers the tool's `title`/`questions[].prompt` or `overview` over plain text (those asks often have no text parts). If that latest turn has no remaining prose and no usable tool preview, the body is `(empty)`. If no Cursor transcript is available, it falls back to the terminal/session title. The Cursor session id is taken from the live chat DB opened by that pane's agent process when possible (so `/clear` does not leave a stale Herdr `agent_session`), then falls back to Herdr's reported session. Other agents always use the session title.
